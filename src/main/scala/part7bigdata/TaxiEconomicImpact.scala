...
object TaxiEconomicImpact {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (args.length != 3) {
      println(
        "Need 1) big data source, 2) taxi zones data source, 3) output data destination"
      )
      System.exit(1)
    }
    ...
    val groupAttemptsDF = bigTaxiDF
      .select(
        round(unix_timestamp(col("pickup_datetime")) / 300)
          .cast("integer")
          .as("fiveMinId"),
      ...
      .agg(
        (count("*") * percentGroupable).as("total_trips"),
    ...
    val totalEconomicImpactDF = groupingEstimateEconomicImpactDF.select(
